...
/*void Raytracer::Render(void)
{
	...
	Vector screenpos, currline = topleft, dir, samplepos, sampleline;

	int numSamples = multisampling;
	Vector ipdx = dx / lrflti(numSamples);
	Vector ipdy = dy / lrflti(numSamples);
	Vector startOffset = ((dx + dy) * lrflti(numSamples - 1)) * (-1.0f / lrflti(-2 * numSamples));
	Color accumulator;
	float divider = 1.0f / lrflti(numSamples * numSamples);
	...
	int y, x, xs, ys;
	#pragma omp parallel for default(none) shared(height, width, dy, dx, o, surface, topleft, startOffset, ipdx, ipdy, numSamples, divider) private(x, y, screenpos, pixel, dir, geom, xs, ys, sampleline, samplepos, accumulator) firstprivate(r, dist) schedule(dynamic, 1)
	for(y = 0; y < height; y++)
	{
		...
		for(x = 0; x < width; x++, screenpos += dx)
		{
			...
			sampleline = screenpos + startOffset;
			accumulator = Color::black;
			for(ys = 0; ys < numSamples; ys++, sampleline += ipdy)
			{
				samplepos = sampleline;
				for(xs = 0; xs < numSamples; xs++, samplepos += ipdx)
				{
					//clear out the pixel
					pixel = Color::black;
					//create the ray
